...
def pairwise_distance(point_cloud,mask=None):
  ...
  Args:
    ...
    mask       : tensor (batch_size, num_points)  
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
      pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, 4, 5))
      mask_zeros = tf.placeholder(tf.float32, shape=(batch_size, 4))
      pair = pairwise_distance(pointclouds_pl[:,:,:3],mask_zeros)
      nn_idx = knn(pair, k=k)
